...
(async function main() {
    try {
        ...
        if (!zipPath) {
            ...
            // Create a zip
            await exec.exec(`zip -r -y ${themeZip} ${core.getInput('working-directory') || '.'} -x *.git* *.zip yarn* npm* node_modules* *routes.yaml *redirects.yaml *redirects.json ${exclude}`, [], {cwd: basePath});
        }
